...
const getStatusChip = (status) => {
  ...
  switch (normalized) {
    case "applied":
      return (
        <Chip variant="bordered" className="border-zinc-600 text-zinc-100 text-xs font-medium px-3 py-1">
          Applied
        </Chip>
      );
    case "review":
      return (
        <Chip variant="bordered" className="border-amber-700 text-amber-500 text-xs font-medium px-3 py-1 bg-amber-950/50">
          Review
        </Chip>
      );
    case "shortlisted":
      return (
        <Chip variant="bordered" className="border-emerald-600/70 text-emerald-500 text-xs font-medium px-3 py-1 bg-emerald-950/50">
          Shortlisted
        </Chip>
      );
    case "rejected":
      return (
        <Chip variant="bordered" className="border-rose-700/70 text-rose-600 text-xs font-medium px-3 py-1 bg-rose-950/50">
          Rejected
        </Chip>
      );
    case "offered":
      return (
        <Chip variant="bordered" className="border-zinc-400 text-zinc-200 text-xs font-medium px-3 py-1">
          Offered
        </Chip>
      );
    default:
      return (
        <Chip variant="bordered" className="border-zinc-600 text-zinc-100 text-xs font-medium">
          {status}
        </Chip>
      );
  }
...
const ApplicationsTablePage = ({ jobs }) => {

    console.log(jobs, "My applied jobs ");
  return (
    <div className="w-full bg-[#121212] p-6 rounded-xl border border-zinc-800/80 min-h-screen text-zinc-100">
      <h2 className="text-xl font-semibold mb-6 text-zinc-200">Applications ({jobs.length})</h2>
                  ...
                  <Table.Row key={job._id?.$oid || job.jobId}>
                    ...
                    <Table.Cell>
                      <div className="flex items-center gap-4">
                        <div className={`p-2.5 rounded-lg flex items-center justify-center ${style.bg}`}>{style.icon}</div>
                        <div className="flex flex-col gap-0.5">
                          <span className="font-medium text-zinc-100 text-[15px]">{job.jobTitle}</span>
                    ...
                    {/* COMPANY COLUMN */}
                    <Table.Cell>
                      <span className="text-zinc-300 text-[14px]">{job.companyName}</span>
                    </Table.Cell>

                    {/* APPLIED COLUMN */}
                    <Table.Cell>
                      <span className="text-zinc-400 text-[14px]">{formatRelativeTime(job.createdAt?.$date || job.appliedAt)}</span>
                    </Table.Cell>

                    {/* STATUS COLUMN */}
                    <Table.Cell>{getStatusChip(job?.status || "Applied")}</Table.Cell>

                    {/* ACTION COLUMN */}
                    <Table.Cell className="text-right">
                      <Button size="sm" variant="light" className="text-zinc-300 hover:text-zinc-100 font-normal text-sm" onClick={() => console.log("Details for:", job.jobId)}>
                        Details
                      </Button>
                    </Table.Cell>
                  </Table.Row>
