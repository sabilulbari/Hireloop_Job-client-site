...
export default function JobCard({ job }) {
  ...
  const salaryRange = job.minSalary && job.maxSalary ? `$${formatSalary(job.minSalary)}–$${formatSalary(job.maxSalary)} / year` : "Salary Negociable";
    ...
    <Card className="p-6 w-full max-w-[440px] border-none bg-zinc-900 text-zinc-100 rounded-[32px] shadow-2xl">
      ...
      <Card.Header className="flex flex-col items-start gap-4 p-0 pb-3">
        <div className="flex items-center gap-3">
          {job.companyLogo && <img src={job.companyLogo} alt={`${job.companyName || "Company"} logo`} className="w-8 h-8 object-contain rounded-md" />}
          <span className="text-lg font-medium text-zinc-300">{job?.companyName ? job?.companyName :  job?.companyId.charAt(0).toUpperCase() + job.companyId.slice(1) || "Confidential"}</span>
        </div>

        <Card.Title className="text-3xl font-semibold tracking-tight text-white leading-tight">{job.jobTitle}</Card.Title>

        {job.responsibilities && <Card.Description className="text-base text-zinc-400 line-clamp-2">{job.responsibilities}</Card.Description>}
      </Card.Header>
      ...
      <Card.Content className="flex flex-col gap-5 p-0 py-4">
        ...
        {(job.requirements || job.benefits) && (
          <div className="text-xs text-zinc-500 space-y-1 border-t border-zinc-800/60 pt-3">
            {job.requirements && (
              <p>
                <strong className="text-zinc-400">Requirements:</strong> {job.requirements}
              </p>
            )}
            {job.benefits && (
              <p>
                <strong className="text-zinc-400">Benefits:</strong> {job.benefits}
              </p>
            )}
          </div>
        )}
