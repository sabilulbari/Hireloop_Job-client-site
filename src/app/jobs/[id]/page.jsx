        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              {job.companyLogo && (
                <img src={job.companyLogo} alt={`${job.companyId} Branding`} className="w-14 h-14 object-contain bg-zinc-900 border border-zinc-800 p-2 rounded-xl" />
              )}
              <div>
                <h2 className="text-xl font-medium text-zinc-300">
                  {job?.companyName ? job?.companyName : job?.companyId.charAt(0).toUpperCase() + job.companyId.slice(1) || "Confidential"}
                </h2>
                <p className="text-sm text-zinc-500 capitalize">{job.jobCategory} Role</p>
              </div>
            </div>

            <h1 className="text-4xl md:text-5xl font-bold tracking-tight text-white leading-tight">{job.jobTitle}</h1>
          </div>

          {/* Section: Responsibilities */}
          <section className="space-y-3">
            <h3 className="text-xl font-semibold text-white">Core Responsibilities</h3>
            <p className="text-zinc-300 text-base leading-relaxed whitespace-pre-line">{job.responsibilities || "No description responsibilities specified for this listing."}</p>
          </section>
          ...
          <section className="space-y-3">
            ...
            <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-2xl p-5">
              <p className="text-zinc-300 text-base leading-relaxed">{job.requirements || "Standard industry standards apply."}</p>
            </div>
          </section>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <span className="text-sm font-medium text-zinc-200">
                  {job.minSalary && job.maxSalary ? `$${formatSalary(job.minSalary)} – $${formatSalary(job.maxSalary)} / year` : "Competitive Salary"}
                </span>
              </div>
            </div>
