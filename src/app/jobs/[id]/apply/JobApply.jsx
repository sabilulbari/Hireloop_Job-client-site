...
import { ArrowRight, FileText, LayoutHeaderCells } from '@gravity-ui/icons';
...
const JobApply = ({ job, applicant }) => {
      ...
      const handleChange = (e) => {
       ...
       const handleSubmit = async (e) => {
        ...
        const submissionData = {
          ...
          companyName: job?.companyName ? job?.companyName : job?.companyId.charAt(0).toUpperCase() + job.companyId.slice(1),
    ...
    return (
      <div className="max-w-xl mx-auto my-8 p-6 bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-800">
        {/* Header Section */}
        <div className="mb-6">
          <span className="text-xs font-semibold text-blue-600 dark:text-blue-400 uppercase tracking-wider">Application Form</span>
          <h2 className="text-2xl font-bold text-zinc-900 dark:text-zinc-50 mt-1">Apply for {job?.title || "this position"}</h2>
          {applicant?.name && (
        ...
        <Form onSubmit={handleSubmit} className="flex flex-col gap-5 w-full">
          ...
          <TextField isRequired name="resumeLink" className="w-full">
            ...
            />
            <Description className="text-xs text-zinc-400 dark:text-zinc-500 mt-1">Provide a public link to your resume (Google Drive, Notion, Dropbox, etc.)</Description>
          ...
          <TextField name="portfolioLink" className="w-full">
            ...
            <Input
              ...
              className="w-full px-3 py-2 rounded-lg border border-zinc-300 dark:border-zinc-700 bg-transparent text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 transition"
            />
            <Description className="text-xs text-zinc-400 dark:text-zinc-500 mt-1">Link to your GitHub, Behance, or personal website.</Description>
          </TextField>
          ...
          <div className="flex items-center justify-end gap-3 mt-4 pt-4 border-t border-zinc-100 dark:border-zinc-800/50">
            <Button
              ...
              className="px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 rounded-lg transition"
            >
              Clear Form
            </Button>
            <Button type="submit" className="px-5 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-sm flex items-center gap-2 transition">
              Submit Application
